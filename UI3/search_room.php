...
<nav>
        ...
        <ul>
                ...
                <li class="dropdown-link">
                    ...
                    class="btn btn-outline-light text-dark" style="text-align: center; width: 100%; font-size: 100%;">การจองของฉัน</button></a></li>
                        <li><a href="index.php"><button type="button" name="logout"
    ...
    <div class="cover2">
        <br>
        <h1 style="text-align: center;">ผลการค้นหาโรงแรม</h1>
        <br>
        <section class="center">
            ...
            <?php
                ...
                if ($_SERVER["REQUEST_METHOD"] == "POST") {
                    ...
                    $sql = "SELECT * FROM (
                        SELECT hotel.hotel_id, room.room_id, hotel.hotel_name, photo.photo_url, locations.address, MIN(roomtype.room_price) AS min_price FROM room
                        ...
                        WHERE roomtype.room_numtotal >= '$quantityRoom' 
                        AND '$quantityAdults' <= roomtype.max_person * '$quantityRoom' 
                        AND locations.province = '$going_to' 
                        GROUP BY hotel.hotel_id, room.room_id
                    ) AS available
                    LEFT JOIN (
                        SELECT DISTINCT room.room_id FROM room
                        LEFT OUTER JOIN booking
                        ON room.hotel_id = booking.hotel_id AND room.room_id = booking.room_id
                        WHERE booking.check_in_time < '$ldate' AND booking.check_out_time >= '$fdate'
                    ) AS unavailable ON available.room_id = unavailable.room_id
                    WHERE unavailable.room_id IS NULL
                    GROUP BY available.hotel_id;";
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            // แสดงข้อมูลของโรงแรม
                            echo '<div>';
                            echo '<a href="each_hotel.php?hotel_id=' . $row["hotel_id"] . '">' . '<img src="' . $row["photo_url"] . '" alt="' . '" class="hotels". id = "hotel_img"."width="300px" height="300px""></a>';
                            echo '<p class="hotels" id="hotel_des">' ."<span style='display: block; text-align: left;'>". $row["hotel_name"] . "<br>". $row["address"] ."<br>"."<span style='color: red;'>"."ราคา(ต่ำสุด) " . $row["min_price"] . 'บาท</p>'."</span>"."</span>";
                            echo '</div>';
